// npx @gltf-transform/cli optimize hershel.glb hershel-optimized.glb --compress draco --texture-compress webp

import { Renderer, Camera, Transform, Orbit, Program, DracoManager, GLTFLoader, Vec3, TextureLoader, Mesh } from 'ogl';
...

{
    ...
    async function loadInitial() {
        // Set draco manager and pass in draco worker script location
        // Note: this is the only added line from the load-gltf example
        GLTFLoader.setDracoManager(new DracoManager(`assets/libs/draco/DracoWorker.js`));

        gltf = await GLTFLoader.load(gl, `assets/gltf/hershel-optimized.glb`);
        addGLTF(gltf);
    }
